package io.github.c_caelum.focalworks.config
...
object FocalworksClientConfig {
    ...
    fun init() {
        ...
        holder.registerSaveListener { _, config ->
            FocalworksServerConfig.holder.config = FocalworksServerConfig.GlobalConfig(config.server)
            InteractionResult.PASS
        }

        // when we leave a server, clear our local copy of its config
        ClientPlayerEvent.CLIENT_PLAYER_QUIT.register { _ ->
            FocalworksServerConfig.onSyncConfig(null)
        }
    }
    ...
    class GlobalConfig : GlobalData() {
        ...
        // this should only be used inside of this class; use FocalworksServerConfig.config to access the server-side config in other code
        @Category("server")
        @TransitiveObject
        val server = FocalworksServerConfig.ServerConfig()
    }
